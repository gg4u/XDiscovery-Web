...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding model 'Topic'
        db.create_table(u'xdw_core_topic', (
            ('topic', self.gf('django.db.models.fields.CharField')(max_length=500, primary_key=True)),
        ))
        for mt in orm.MapTopic.objects.all():
            if not db.dry_run:
                orm.Topic.objects.get_or_create(topic=mt.topic)

        db.send_create_signal(u'xdw_core', ['Topic'])


    def backwards(self, orm):
        # Deleting model 'Topic'
        db.delete_table(u'xdw_core_topic')
